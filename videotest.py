...
# Load video
video = cv2.VideoCapture("aruco_videos/CCW_rev_1.mp4")
        ...
        if len(tvecs) >= 3:
            ...
            if contours:
                ...
                if M["m00"] != 0:
                    ...
                    t = (d - np.dot(plane_normal, camera_origin)) / np.dot(plane_normal, ray_direction)
                    ...
                    if target_id in ids:
                        ...
                        yellow_ball_homogeneous = np.append(yellow_ball_3d_position, 1)
                        yellow_ball_position_marker_coords = inverse_transformation @ yellow_ball_homogeneous
                        yellow_ball_position_marker_coords = yellow_ball_position_marker_coords[:3]

                        # Format XYZ coordinates for display
                        ...
                        # Display the XYZ coordinates on the frame next to the yellow ball
                        text_position = (yellow_center[0] + 10, yellow_center[1] - 10)
                        cv2.putText(frame, label_text, text_position,
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                    else:
                        print(f"Marker ID {target_id} not found in the frame.")
